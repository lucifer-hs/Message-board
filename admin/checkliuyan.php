      ...
      <div class="main">
        ...
        <div class="side">
          <?php
            ...
            $ckeck=$_SESSION['check'];     
            //var_dump($_SESSION['ckeck']);
            $result=mysql_query("select*from image where username='$ckeck' order by userid desc");
            echo "<table>";
            echo "<p>";
            ...
            echo "<a href='delcliuyan.php?userid=0' style='float: right;margin-right: 50px;margin-top:10px;'>删除所有留言</a>";
            echo "</p>";
            while($row=mysql_fetch_array($result))
            {
              $id=$row['userid'];
              echo "<tr>";
              echo "<td style='margin-top: 60px;'>用户：".$row['username']." || 标题：".$row['biaoti']."<a href='delcliuyan.php?userid=$id' style='float: right;'>删除留言</a>"."</td>";
              echo "</tr>";
              echo "<tr>";
